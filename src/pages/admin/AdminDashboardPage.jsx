...
const AdminDashboardPage = () => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      ...
      const articleSnapshot = await getDocs(collection(db, 'blogPosts'));
      setArticleCount(articleSnapshot.size);
    };
    fetchCounts();
  }, []);
            ...
            <CardContent className="grid grid-cols-2 gap-4">
              <Link to="/admin/products?action=add" className="block">
                <Button variant="outline" className="w-full justify-start">
                  <Battery className="mr-2 h-4 w-4" /> Tambah Produk
                </Button>
              </Link>
              <Link to="/admin/articles?action=add" className="block">
                <Button variant="outline" className="w-full justify-start">
                  <FileText className="mr-2 h-4 w-4" /> Tambah Artikel
                </Button>
              </Link>
              ...
              <Link to="#" className="block">
                <Button variant="outline" className="w-full justify-start">
                  <BarChart className="mr-2 h-4 w-4" /> Lihat Laporan
                </Button>
              </Link>
            </CardContent>
          </Card>
